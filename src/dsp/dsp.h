...
/* Rust f32::powi: successive squaring, reciprocal for negative n */
float powi_f(float base, int n);
/* x - floor(x); matches Rust's `p -= p.floor()` phase wrap */
...
#define COMP_RELEASE_FAST_MS 60.0f
...
typedef struct {
    ...
    Top top_l, top_r;
    Top even_l, even_r;
    float dc_l0, dc_l1, dc_r0, dc_r1; /* Rust's (f32,f32) dc state pairs */
