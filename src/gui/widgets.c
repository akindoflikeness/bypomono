...
/* dotted border whose dots walk clockwise round the rect */
static void marching_rect(Canvas *c, Rct r, double time, uint8_t ink) {
    const float STEP = 4.0f, DOT = 2.0f, SPEED = 12.0f;
    float w = rct_w(r) - DOT, h = rct_h(r) - DOT;
    float perimeter = 2.0f * (w + h);
    float s = fmodf((float)time * SPEED, STEP);
    for (; s < perimeter; s += STEP) {
        float x, y;
        if (s < w) x = s, y = 0.0f;
        else if (s < w + h) x = w, y = s - w;
        else if (s < 2.0f * w + h) x = w - (s - w - h), y = h;
        else x = 0.0f, y = h - (s - 2.0f * w - h);
        draw_rect_filled(c, rct_xywh(roundf(r.x0 + x), roundf(r.y0 + y), DOT,
                                     DOT), ink);
    }
}

/* console.c declares: extern float hint_chip(Ui *ui, P2 at, const char *text,
   bool highlighted); draws at top-left `at`, HINT_CHIP_H tall, returns
   chip width */
float hint_chip(Ui *ui, P2 at, const char *text, bool highlighted) {
    Canvas *c = ui->canvas;
    FontId f = ui_font(11.0f);
    float w = text_width(f, text, 0.0f) + 2.0f * GAP;
    Rct r = rct_xywh(at.x, at.y, w, HINT_CHIP_H);
    draw_rect_filled(c, r, INK_BLACK);
    if (highlighted) {
        marching_rect(c, r, ui->time, PAPER);
        ui->repaint_soon = true;
    } else {
        hard_rect(c, r, 1.0f);
    }
    float th = text_row_height(f);
    text_draw(c, f,
              (P2){roundf(r.x0 + GAP), roundf(0.5f * (r.y0 + r.y1) - th * 0.5f)},
              ALIGN_LEFT_TOP, text, PAPER, 0.0f);
    return w;
}
