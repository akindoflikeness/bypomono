...
static const char *stock_dir(void) {
    static char buf[PATHBUF];
    if (is_dir_path("crates/fibonacci-gui")) {
        snprintf(buf, sizeof buf, "crates/fibonacci-gui/presets");
    } else if (exe_dir()) {
...
const char *asset_dir(void) {
    ...
    static bool done = false;
    if (!done) {
        snprintf(buf, sizeof buf, "crates/fibonacci-gui/assets");
        if (!is_dir_path(buf)) {
...
static bool load_saved(const char *path, Session *out) {
...
bool app_restore_state(App *a) {
    ...
    Session s;
    bool restored = load_saved(state_path, &s);
    ...
    if (!restored) {
        ...
        if (!load_saved(def_path, &s)) s = compiled_init();
    }
...
void preset_load(App *a, const PresetRef *r) {
    char path[PATHBUF];
    preset_path(r, path, sizeof path);
    Session s;
    if (load_saved(path, &s)) {
        app_apply_session(a, s);
        a->preset_loaded = *r;
        a->have_loaded = true;
...
static bool save_in(App *a, const char *folder, const char *name,
    ...
    char dir[JOINBUF];
    if (folder && folder[0])
        snprintf(dir, sizeof dir, "%s/%.127s", preset_dir(), folder);
    else
    ...
    preset_path(&written, path, sizeof path);
