...
static void rip_line_init(RipLine *r, float sample_rate) {
    ...
    r->write = 0;
    r->delay = delay;
    phase_rotator_init(&r->rot, allpass_coeff_for(DESIGN_HZ, sample_rate, PHASE_PER_PASS));
    r->lp = 0.0f;
    r->fb = 0.0f;
}
...
void voice_init(Voice *v, float sample_rate, Patch patch) {
    ...
    rip_line_init(&v->rip_line, sample_rate);
    v->rip_sig = 0.0f;
