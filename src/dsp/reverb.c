...
static float comb_process(Comb *c, float x, float damp) {
    ...
    size_t len = c->len;
    float d = clampf((float)c->delay + wobble, 1.0f, (float)(len - 2));
    size_t di = (size_t)d;
...
static float ghost_process(GhostLine *g, float x) {
    size_t read = (g->write + g->len - g->delay) % g->len;
    float y = phase_rotator_process(&g->rot, g->buf[read]);
...
void verb_init(StereoVerb *v, float sample_rate) {
    ...
    float ghost_a = allpass_coeff_for(DESIGN_HZ, sample_rate, PHASE_PER_PASS);
...
void verb_configure(StereoVerb *v, const Patch *patch, const Compiled *compiled) {
    for (int i = 0; i < NUM_OPS; i++) {
        ...
        v->send[i] = carrier ? 1.0f : powi_f(PHI, -((int)compiled->depth[i] - 1));
    }
}
...
Stereo verb_process(StereoVerb *v, const Frame *frame) {
    ...
    float sends[NUM_OPS];
    for (int i = 0; i < NUM_OPS; i++) {
        float bleed = v->is_carrier[i] ? 1.0f : v->ghost_s;
    ...
    for (int i = 0; i < NUM_OPS; i++) {
        ...
        wet_l += comb_process(&v->combs_l[i], x, loop_damp);
        wet_r += comb_process(&v->combs_r[i], x, loop_damp);
    }
