...
/* console.c declares: extern float hint_chip(Ui *ui, P2 at, const char *text,
   bool lit); draws at top-left `at`, HINT_CHIP_H tall, returns chip width */
float hint_chip(Ui *ui, P2 at, const char *text, bool lit) {
    Canvas *c = ui->canvas;
    uint8_t fill = lit ? PAPER : INK_BLACK;
    uint8_t ink = lit ? INK_BLACK : PAPER;
    FontId f = ui_font(11.0f);
