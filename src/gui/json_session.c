#include "app.h"

#include <math.h>
#include <stdlib.h>
#include <string.h>

typedef struct {
    const char *p, *end;
    bool err;
} Js;

static void js_ws(Js *j) {
    while (j->p < j->end &&
           (*j->p == ' ' || *j->p == '\t' || *j->p == '\n' || *j->p == '\r'))
        j->p++;
}

static bool js_ch(Js *j, char c) {
    js_ws(j);
    if (j->p < j->end && *j->p == c) {
        j->p++;
        return true;
    }
    return false;
}

static bool js_lit(Js *j, const char *s) {
    size_t n = strlen(s);
    if ((size_t)(j->end - j->p) >= n && memcmp(j->p, s, n) == 0) {
        j->p += n;
        return true;
    }
    return false;
}

static int hex_val(char c) {
    if (c >= '0' && c <= '9') return c - '0';
    if (c >= 'a' && c <= 'f') return c - 'a' + 10;
    if (c >= 'A' && c <= 'F') return c - 'A' + 10;
    return -1;
}

static bool js_string(Js *j, char *out, size_t cap) {
    js_ws(j);
    if (j->p >= j->end || *j->p != '"') {
        j->err = true;
        return false;
    }
    j->p++;
    size_t n = 0;
    while (j->p < j->end) {
        char c = *j->p++;
        if (c == '"') {
            if (out && cap) out[n < cap ? n : cap - 1] = 0;
            return true;
        }
        if (c == '\\') {
            if (j->p >= j->end) break;
            char e = *j->p++;
            char put = 0;
            switch (e) {
            case '"': put = '"'; break;
            case '\\': put = '\\'; break;
            case '/': put = '/'; break;
            case 'b': put = '\b'; break;
            case 'f': put = '\f'; break;
            case 'n': put = '\n'; break;
            case 'r': put = '\r'; break;
            case 't': put = '\t'; break;
            case 'u': {
                unsigned cp = 0;
                for (int k = 0; k < 4; k++) {
                    int h = j->p < j->end ? hex_val(*j->p) : -1;
                    if (h < 0) {
                        j->err = true;
                        return false;
                    }
                    cp = cp * 16 + (unsigned)h;
                    j->p++;
                }
                char enc[4];
                int en;
                if (cp < 0x80) {
                    enc[0] = (char)cp;
                    en = 1;
                } else if (cp < 0x800) {
                    enc[0] = (char)(0xC0 | (cp >> 6));
                    enc[1] = (char)(0x80 | (cp & 0x3F));
                    en = 2;
                } else {
                    enc[0] = (char)(0xE0 | (cp >> 12));
                    enc[1] = (char)(0x80 | ((cp >> 6) & 0x3F));
                    enc[2] = (char)(0x80 | (cp & 0x3F));
                    en = 3;
                }
                for (int k = 0; k < en; k++) {
                    if (out && n + 1 < cap) out[n] = enc[k];
                    n++;
                }
                continue;
            }
            default: j->err = true; return false;
            }
            if (out && n + 1 < cap) out[n] = put;
            n++;
            continue;
        }
        if (out && n + 1 < cap) out[n] = c;
        n++;
    }
    j->err = true;
    return false;
}

static bool js_number(Js *j, double *out) {
    js_ws(j);
    char *endp;
    double v = strtod(j->p, &endp);
    if (endp == j->p) {
        j->err = true;
        return false;
    }
    j->p = endp;
    *out = v;
    return true;
}

static void js_skip(Js *j);

static void js_skip_object(Js *j) {
    if (js_ch(j, '}')) return;
    for (;;) {
        if (!js_string(j, NULL, 0)) return;
        if (!js_ch(j, ':')) {
            j->err = true;
            return;
        }
        js_skip(j);
        if (j->err) return;
        if (js_ch(j, ',')) continue;
        if (js_ch(j, '}')) return;
        j->err = true;
        return;
    }
}

static void js_skip_array(Js *j) {
    if (js_ch(j, ']')) return;
    for (;;) {
        js_skip(j);
        if (j->err) return;
        if (js_ch(j, ',')) continue;
        if (js_ch(j, ']')) return;
        j->err = true;
        return;
    }
}

static void js_skip(Js *j) {
    js_ws(j);
    if (j->p >= j->end) {
        j->err = true;
        return;
    }
    char c = *j->p;
    if (c == '"') {
        js_string(j, NULL, 0);
    } else if (c == '{') {
        j->p++;
        js_skip_object(j);
    } else if (c == '[') {
        j->p++;
        js_skip_array(j);
    } else if (c == 't') {
        if (!js_lit(j, "true")) j->err = true;
    } else if (c == 'f') {
        if (!js_lit(j, "false")) j->err = true;
    } else if (c == 'n') {
        if (!js_lit(j, "null")) j->err = true;
    } else {
        double d;
        js_number(j, &d);
    }
}

static int js_obj_next(Js *j, bool *first, char *key, size_t cap) {
    js_ws(j);
    if (js_ch(j, '}')) return 0;
    if (!*first && !js_ch(j, ',')) {
        j->err = true;
        return -1;
    }
    *first = false;
    if (!js_string(j, key, cap)) return -1;
    if (!js_ch(j, ':')) {
        j->err = true;
        return -1;
    }
    return 1;
}

static float js_f32(Js *j, float dflt) {
    double d;
    if (js_number(j, &d)) return (float)d;
    return dflt;
}

static bool js_bool(Js *j, bool dflt) {
    js_ws(j);
    if (js_lit(j, "true")) return true;
    if (js_lit(j, "false")) return false;
    j->err = true;
    return dflt;
}

static int js_enum(Js *j, const char *const *names, int n, int dflt) {
    char s[64];
    if (!js_string(j, s, sizeof s)) return dflt;
    for (int i = 0; i < n; i++)
        if (strcmp(s, names[i]) == 0) return i;
    return dflt;
}

static const char *const RATIO_MODE_NAMES[] = {
    "Harmonic", "Fibonacci", "Golden", "GoldenMirror", "Plastic"};
static const char *const SCALE_NAMES[] = {
    "Phrygian",        "PhrygianDominant", "NaturalMinor", "HarmonicMinor",
    "NeapolitanMinor", "Byzantine",        "Fibonacci",    "Phyllotaxis"};
static const char *const TUNING_NAMES[] = {
    "Scale", "FibonacciHz", "GoldenPowers", "PlasticPowers", "GoldenWalk"};
static const char *const HOLD_NAMES[] = {"GoldenWeyl", "Xorshift"};

static void parse_ops(Js *j, OpParams ops[NUM_OPS]) {
    if (!js_ch(j, '[')) {
        j->err = true;
        return;
    }
    if (js_ch(j, ']')) return;
    int i = 0;
    for (;;) {
        if (i < NUM_OPS) {
            if (!js_ch(j, '{')) {
                j->err = true;
                return;
            }
            OpParams *op = &ops[i];
            bool first = true;
            char key[64];
            int r;
            while ((r = js_obj_next(j, &first, key, sizeof key)) == 1) {
                if (strcmp(key, "enabled") == 0)
                    op->enabled = js_bool(j, op->enabled);
                else if (strcmp(key, "ratio") == 0)
                    op->ratio = js_f32(j, op->ratio);
                else if (strcmp(key, "detune_cents") == 0)
                    op->detune_cents = js_f32(j, op->detune_cents);
                else if (strcmp(key, "level") == 0)
                    op->level = js_f32(j, op->level);
                else
                    js_skip(j);
                if (j->err) return;
            }
            if (r < 0) return;
        } else {
            js_skip(j);
            if (j->err) return;
        }
        i++;
        if (js_ch(j, ',')) continue;
        if (js_ch(j, ']')) return;
        j->err = true;
        return;
    }
}

static void parse_patch(Js *j, Patch *p) {
    if (!js_ch(j, '{')) {
        j->err = true;
        return;
    }
    p->field = 0.0f;
    p->curve = 0.5f;
    bool first = true;
    char key[64];
    int r;
    while ((r = js_obj_next(j, &first, key, sizeof key)) == 1) {
        if (strcmp(key, "algorithm") == 0) {
            js_ws(j);
            if (j->p < j->end && *j->p == '"') {
                char g[16];
                if (js_string(j, g, sizeof g)) {
                    AlgorithmId id;
                    if (strlen(g) == NUM_NODES && algorithm_from_glyphs(g, &id))
                        p->algorithm = id;
                }
            } else {
                double d;
                if (js_number(j, &d)) {
                    int64_t v = (int64_t)d;
                    if (v < 0) v = 0;
                    p->algorithm =
                        algorithm_from_legacy_bits((uint8_t)(v & 0x0F));
                }
            }
        } else if (strcmp(key, "ratio_mode") == 0) {
            p->ratio_mode = (RatioMode)js_enum(j, RATIO_MODE_NAMES,
                                              RATIO_MODE_COUNT, RATIO_GOLDEN);
        } else if (strcmp(key, "ops") == 0) {
            parse_ops(j, p->ops);
        } else if (strcmp(key, "feedback") == 0) {
            p->feedback = js_f32(j, p->feedback);
        } else if (strcmp(key, "index") == 0) {
            p->index = js_f32(j, p->index);
        } else if (strcmp(key, "rip") == 0) {
            p->rip = js_f32(j, p->rip);
        } else if (strcmp(key, "master_level") == 0) {
            p->master_level = js_f32(j, p->master_level);
        } else if (strcmp(key, "glide_seconds") == 0) {
            p->glide_seconds = js_f32(j, p->glide_seconds);
        } else if (strcmp(key, "field") == 0) {
            p->field = js_f32(j, p->field);
        } else if (strcmp(key, "curve") == 0) {
            p->curve = js_f32(j, p->curve);
        } else {
            js_skip(j);
        }
        if (j->err) return;
    }
}

static void parse_verb(Js *j, VerbParams *v) {
    if (!js_ch(j, '{')) {
        j->err = true;
        return;
    }
    bool first = true;
    char key[64];
    int r;
    while ((r = js_obj_next(j, &first, key, sizeof key)) == 1) {
        if (strcmp(key, "mix") == 0)
            v->mix = js_f32(j, v->mix);
        else if (strcmp(key, "ghost") == 0)
            v->ghost = js_f32(j, v->ghost);
        else if (strcmp(key, "decay") == 0 || strcmp(key, "rt60") == 0)
            v->decay = js_f32(j, v->decay);
        else if (strcmp(key, "damp") == 0)
            v->damp = js_f32(j, v->damp);
        else if (strcmp(key, "haunt") == 0)
            v->haunt = js_f32(j, v->haunt);
        else
            js_skip(j);
        if (j->err) return;
    }
}

static uint8_t js_u8(Js *j, uint8_t dflt) {
    double d;
    if (!js_number(j, &d)) return dflt;
    if (d < 0.0) d = 0.0;
    if (d > 255.0) d = 255.0;
    return (uint8_t)d;
}

static void parse_melody(Js *j, MelodyParams *m) {
    if (!js_ch(j, '{')) {
        j->err = true;
        return;
    }
    bool first = true;
    char key[64];
    int r;
    while ((r = js_obj_next(j, &first, key, sizeof key)) == 1) {
        if (strcmp(key, "enabled") == 0)
            m->enabled = js_bool(j, m->enabled);
        else if (strcmp(key, "tuning") == 0)
            m->tuning =
                (Tuning)js_enum(j, TUNING_NAMES, NUM_TUNINGS, TUNING_SCALE);
        else if (strcmp(key, "scale") == 0)
            m->scale =
                (Scale)js_enum(j, SCALE_NAMES, NUM_SCALES, SCALE_PHRYGIAN);
        else if (strcmp(key, "root_midi") == 0)
            m->root_midi = js_u8(j, m->root_midi);
        else if (strcmp(key, "range_degrees") == 0)
            m->range_degrees = js_u8(j, m->range_degrees);
        else if (strcmp(key, "rate_hz") == 0)
            m->rate_hz = js_f32(j, m->rate_hz);
        else if (strcmp(key, "source") == 0)
            m->source = (HoldSource)js_enum(j, HOLD_NAMES, 2, HOLD_GOLDEN_WEYL);
        else
            js_skip(j);
        if (j->err) return;
    }
}

static void parse_chandas(Js *j, ChandasParams *c) {
    if (!js_ch(j, '{')) {
        j->err = true;
        return;
    }
    bool first = true;
    char key[64];
    int r;
    while ((r = js_obj_next(j, &first, key, sizeof key)) == 1) {
        if (strcmp(key, "enabled") == 0)
            c->enabled = js_bool(j, c->enabled);
        else if (strcmp(key, "mix") == 0)
            c->mix = js_f32(j, c->mix);
        else if (strcmp(key, "sync") == 0)
            c->sync = js_bool(j, c->sync);
        else if (strcmp(key, "division") == 0) {
            double d;
            if (js_number(j, &d)) c->division = d < 0.0 ? 0 : (size_t)d;
        } else if (strcmp(key, "rate_hz") == 0)
            c->rate_hz = js_f32(j, c->rate_hz);
        else if (strcmp(key, "spread") == 0)
            c->spread = js_f32(j, c->spread);
        else if (strcmp(key, "size") == 0)
            c->size = js_f32(j, c->size);
        else if (strcmp(key, "warp") == 0)
            c->warp = js_f32(j, c->warp);
        else if (strcmp(key, "dimension") == 0)
            c->dimension = js_f32(j, c->dimension);
        else if (strcmp(key, "tail") == 0)
            c->tail = js_f32(j, c->tail);
        else
            js_skip(j);
        if (j->err) return;
    }
}

bool session_from_json(const char *json, Session *out) {
    if (!json || !out) return false;
    Js j = {json, json + strlen(json), false};
    Session s = session_default();
    s.warmth = 0.0f;
    s.tempo_bpm = CHANDAS_DEFAULT_BPM;
    s.chandas = chandas_params_default();
    if (!js_ch(&j, '{')) return false;
    bool first = true;
    char key[64];
    int r;
    while ((r = js_obj_next(&j, &first, key, sizeof key)) == 1) {
        if (strcmp(key, "patch") == 0)
            parse_patch(&j, &s.patch);
        else if (strcmp(key, "verb") == 0)
            parse_verb(&j, &s.verb);
        else if (strcmp(key, "melody") == 0)
            parse_melody(&j, &s.melody);
        else if (strcmp(key, "drone_hz") == 0)
            s.drone_hz = js_f32(&j, s.drone_hz);
        else if (strcmp(key, "chandas") == 0 || strcmp(key, "harmony") == 0)
            parse_chandas(&j, &s.chandas);
        else if (strcmp(key, "tempo_bpm") == 0)
            s.tempo_bpm = js_f32(&j, s.tempo_bpm);
        else if (strcmp(key, "warmth") == 0)
            s.warmth = js_f32(&j, s.warmth);
        else
            js_skip(&j);
        if (j.err) return false;
    }
    if (r < 0 || j.err) return false;
    *out = session_sanitize(s);
    return true;
}

typedef struct {
    char *buf;
    size_t len, cap;
} Sb;

static void sb_put(Sb *b, const char *s) {
    size_t n = strlen(s);
    if (b->len + n + 1 > b->cap) {
        size_t c = b->cap ? b->cap * 2 : 512;
        while (c < b->len + n + 1) c *= 2;
        char *nb = realloc(b->buf, c);
        if (!nb) return;
        b->buf = nb;
        b->cap = c;
    }
    memcpy(b->buf + b->len, s, n);
    b->len += n;
    b->buf[b->len] = 0;
}

/* shortest round-trip digits, laid out ryu/Rust-Display style */
static void fmt_f32(char *out, size_t cap, float v) {
    char tmp[48];
    if (isnan(v) || isinf(v)) v = 0.0f;
    int p = 9;
    for (int q = 1; q <= 9; q++) {
        snprintf(tmp, sizeof tmp, "%.*e", q - 1, (double)v);
        if (strtof(tmp, NULL) == v) {
            p = q;
            break;
        }
    }
    snprintf(tmp, sizeof tmp, "%.*e", p - 1, (double)v);
    const char *sign = tmp[0] == '-' ? "-" : "";
    const char *t = tmp[0] == '-' ? tmp + 1 : tmp;
    char digits[16];
    int nd = 0;
    digits[nd++] = t[0];
    for (const char *q = t + 1; *q && *q != 'e'; q++)
        if (*q != '.') digits[nd++] = *q;
    digits[nd] = 0;
    int ex = atoi(strchr(t, 'e') + 1);
    char body[48];
    if (ex < -5 || ex > 15) {
        if (nd > 1)
            snprintf(body, sizeof body, "%c.%se%d", digits[0], digits + 1, ex);
        else
            snprintf(body, sizeof body, "%ce%d", digits[0], ex);
    } else if (ex >= nd - 1) {
        int zeros = ex - (nd - 1);
        snprintf(body, sizeof body, "%s%.*s.0", digits, zeros,
                 "000000000000000");
    } else if (ex >= 0) {
        snprintf(body, sizeof body, "%.*s.%s", ex + 1, digits,
                 digits + ex + 1);
    } else {
        snprintf(body, sizeof body, "0.%.*s%s", -ex - 1, "000000000000000",
                 digits);
    }
    snprintf(out, cap, "%s%s", sign, body);
}

static void sb_key_f(Sb *b, const char *ind, const char *key, float v,
                     bool comma) {
    char num[64], line[192];
    fmt_f32(num, sizeof num, v);
    snprintf(line, sizeof line, "%s\"%s\": %s%s\n", ind, key, num,
             comma ? "," : "");
    sb_put(b, line);
}

static void sb_key_s(Sb *b, const char *ind, const char *key, const char *v,
                     bool comma) {
    char line[192];
    snprintf(line, sizeof line, "%s\"%s\": \"%s\"%s\n", ind, key, v,
             comma ? "," : "");
    sb_put(b, line);
}

static void sb_key_b(Sb *b, const char *ind, const char *key, bool v,
                     bool comma) {
    char line[192];
    snprintf(line, sizeof line, "%s\"%s\": %s%s\n", ind, key,
             v ? "true" : "false", comma ? "," : "");
    sb_put(b, line);
}

static void sb_key_u(Sb *b, const char *ind, const char *key, unsigned long v,
                     bool comma) {
    char line[192];
    snprintf(line, sizeof line, "%s\"%s\": %lu%s\n", ind, key, v,
             comma ? "," : "");
    sb_put(b, line);
}

char *session_to_json(const Session *s) {
    Sb b = {NULL, 0, 0};
    char glyphs[NUM_NODES + 1];
    algorithm_to_glyphs(s->patch.algorithm, glyphs);

    sb_put(&b, "{\n");
    sb_put(&b, "  \"patch\": {\n");
    sb_key_s(&b, "    ", "algorithm", glyphs, true);
    sb_key_s(&b, "    ", "ratio_mode", RATIO_MODE_NAMES[s->patch.ratio_mode],
             true);
    sb_put(&b, "    \"ops\": [\n");
    for (int i = 0; i < NUM_OPS; i++) {
        const OpParams *op = &s->patch.ops[i];
        sb_put(&b, "      {\n");
        sb_key_b(&b, "        ", "enabled", op->enabled, true);
        sb_key_f(&b, "        ", "ratio", op->ratio, true);
        sb_key_f(&b, "        ", "detune_cents", op->detune_cents, true);
        sb_key_f(&b, "        ", "level", op->level, false);
        sb_put(&b, i + 1 < NUM_OPS ? "      },\n" : "      }\n");
    }
    sb_put(&b, "    ],\n");
    sb_key_f(&b, "    ", "feedback", s->patch.feedback, true);
    sb_key_f(&b, "    ", "index", s->patch.index, true);
    sb_key_f(&b, "    ", "rip", s->patch.rip, true);
    sb_key_f(&b, "    ", "master_level", s->patch.master_level, true);
    sb_key_f(&b, "    ", "glide_seconds", s->patch.glide_seconds, true);
    sb_key_f(&b, "    ", "field", s->patch.field, true);
    sb_key_f(&b, "    ", "curve", s->patch.curve, false);
    sb_put(&b, "  },\n");

    sb_put(&b, "  \"verb\": {\n");
    sb_key_f(&b, "    ", "mix", s->verb.mix, true);
    sb_key_f(&b, "    ", "ghost", s->verb.ghost, true);
    sb_key_f(&b, "    ", "decay", s->verb.decay, true);
    sb_key_f(&b, "    ", "damp", s->verb.damp, true);
    sb_key_f(&b, "    ", "haunt", s->verb.haunt, false);
    sb_put(&b, "  },\n");

    sb_put(&b, "  \"melody\": {\n");
    sb_key_b(&b, "    ", "enabled", s->melody.enabled, true);
    sb_key_s(&b, "    ", "tuning", TUNING_NAMES[s->melody.tuning], true);
    sb_key_s(&b, "    ", "scale", SCALE_NAMES[s->melody.scale], true);
    sb_key_u(&b, "    ", "root_midi", s->melody.root_midi, true);
    sb_key_u(&b, "    ", "range_degrees", s->melody.range_degrees, true);
    sb_key_f(&b, "    ", "rate_hz", s->melody.rate_hz, true);
    sb_key_s(&b, "    ", "source", HOLD_NAMES[s->melody.source], false);
    sb_put(&b, "  },\n");

    sb_key_f(&b, "  ", "drone_hz", s->drone_hz, true);

    sb_put(&b, "  \"chandas\": {\n");
    sb_key_b(&b, "    ", "enabled", s->chandas.enabled, true);
    sb_key_f(&b, "    ", "mix", s->chandas.mix, true);
    sb_key_b(&b, "    ", "sync", s->chandas.sync, true);
    sb_key_u(&b, "    ", "division", (unsigned long)s->chandas.division, true);
    sb_key_f(&b, "    ", "rate_hz", s->chandas.rate_hz, true);
    sb_key_f(&b, "    ", "spread", s->chandas.spread, true);
    sb_key_f(&b, "    ", "size", s->chandas.size, true);
    sb_key_f(&b, "    ", "warp", s->chandas.warp, true);
    sb_key_f(&b, "    ", "dimension", s->chandas.dimension, true);
    sb_key_f(&b, "    ", "tail", s->chandas.tail, false);
    sb_put(&b, "  },\n");

    sb_key_f(&b, "  ", "tempo_bpm", s->tempo_bpm, true);
    sb_key_f(&b, "  ", "warmth", s->warmth, false);
    sb_put(&b, "}");
    return b.buf;
}
